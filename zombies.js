...
Player.prototype.equip = function (itemToEquip) {
  if (itemToEquip instanceof Weapon && this.getPack().includes(itemToEquip)) {
    let indexOfItem2 = this.getPack().indexOf(itemToEquip);
    if (this.equipped) {
      this.getPack().splice(indexOfItem2, 1, this.equipped);
      this.equipped = itemToEquip;
    } else {
      this.getPack().splice(indexOfItem2, 1)
      this.equipped = itemToEquip;
    }
  }
}
...
Player.prototype.equippedWith = function () {
  if (this.equipped) {
    console.log(`${this.name} is equppied with ${this.equipped.name}`)
